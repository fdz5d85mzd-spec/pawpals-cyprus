// Demo seed data — mirrors the TEAMS/MATCHES/FINISHED_MATCHES mock from the
// original football-predictor.jsx prototype, pushed through Postgres + the
// real computeModel instead of being hardcoded in the UI. Used by both the
// local `npm run db:seed` script and the temporary production
// /api/admin/seed-demo endpoint (for when real leagues are between seasons).
// Delete these rows once real fixtures are flowing — see app/api/admin/seed-demo.
import { PrismaClient, Market, Prisma } from "@prisma/client";
import { computeModel, type TeamStats } from "./model";
...
async function upsertTeam(prisma: PrismaClient, id: number, leagueId: number, stats: TeamStats) {
...
export async function seedDemoData(prisma: PrismaClient) {
  ...
  await upsertTeam(prisma, 1, leagueId, ARIS);
  await upsertTeam(prisma, 2, leagueId, PAOK);
