          ...
          {filteredItems.map((item) => {
            ...
            return (
              <article key={item.id} className="match-card group rounded-2xl border border-white/10 bg-[#141416]/95 p-5">
                ...
                <div className="mt-3 flex flex-wrap items-center justify-between gap-1 text-[10px] text-dim"><span>{t("source")}</span><time dateTime={item.frozenAt} suppressHydrationWarning>{t("lockedAt", { date: new Date(item.frozenAt).toLocaleString(locale, { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" }) })}</time></div>
              </article>
            );
          })}
        </div>
