    ...
    <div className="max-w-2xl mx-auto px-5 pt-8 pb-16">
      ...
      {leagues.length === 0 ? (
        <div className="card p-6 text-center">
          <div className="text-sm text-ink font-medium mb-1">Καμία βαθμολογία διαθέσιμη ακόμα</div>
          <div className="text-xs text-dim">Ο scheduler ενημερώνει τις βαθμολογίες καθημερινά.</div>
        </div>
      ) : (
...
async function StandingsBody({
  ...
  return (
    <>
      ...
      <Eyebrow>Πίνακας βαθμολογίας</Eyebrow>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="text-[9px] uppercase tracking-wide font-mono text-dim border-b border-border/60">
              ...
              <th className="text-left py-2.5 px-3">Ομάδα</th>
              <th className="text-center py-2.5 px-2">ΑΓ</th>
              <th className="text-center py-2.5 px-2">Ν</th>
              <th className="text-center py-2.5 px-2">Ι</th>
              <th className="text-center py-2.5 px-2">Η</th>
              <th className="text-center py-2.5 px-2">Γκολ</th>
              <th className="text-center py-2.5 px-3">Β</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r) => (
